...
require_once(__DIR__ . '/../../estimates/new/functions.php');
...
//入金消込->リマインドのキャンセル
if (!empty($_GET['new'])) {
    $sql = sprintf('UPDATE `quotation` SET notice_flag=1 WHERE u_id="%s" AND qu_id="%d"', mysqli_real_escape_string($db, $_SESSION['loginID']), mysqli_real_escape_string($db, $qu_id) );
...
$errMsg = "";
if (!empty($_POST['send2']) && $_POST['send2'] == "請求書反映") {
	// print_r($_POST);
	// exit();
...
//	}
	...
	if ($errMsg == "") {
		...
		$row11 = mysqli_fetch_assoc($record);

		$upd_flag = 0;
		$upd_memo = date('Y年n月j日'). "　請求書の「";

		if ($row11['qu_bunrui'] != $_POST['qu_bunrui']) {
			$upd_memo .= "対象分類";
			$upd_flag = 1;
		}
		if ($row11['qu_custom_name'] != $_POST['qu_custom_name']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "請求先";
		...
		if ($row11['qu_custom_no'] != $_POST['qu_custom_no']) {
			if ($row11['qu_custom_no'] == 0) {
		...
		if ($row11['qu_name'] != $_POST['qu_name']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "工事名称";
		...
		if ($row11['qu_location'] != $_POST['qu_location']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "工事場所";
		...
		if ($row11['qu_paymentDate'] != $_POST['qu_paymentDate']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "支払日";
		...
		$qu_deliveryDate = date('Y-m-d', strtotime(date($row11['qu_deliveryDate'])));
		if ($row11['qu_deliveryDate'] != $qu_deliveryDate) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "受渡期日";
		...
		if ($row11['in_companyName'] != $_POST['in_companyName']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "自社名";
		...
		if ($row11['in_address1'] != $_POST['in_address1']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "都道府県";
		...
		if ($row11['in_address2'] != $_POST['in_address2']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "市区町村";
		...
		if ($row11['in_address3'] != $_POST['in_address3']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "番地";
		...
		if ($row11['in_email'] != $_POST['in_email']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "メールアドレス";
		...
		if ($row11['in_contactName'] != $_POST['in_contactName']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "担当者名";
...
//		if ($row11['q_alltotal'] != $q_alltotal) {
//			if ($upd_flag == 1) {
//				$upd_memo .= "、";
//			}
//			$upd_memo .= "請求金額";
...
//			$upd_flag = 1;
//		}
		if ($row11['qu_commission'] != $_POST['qu_commission']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "割賦手数料率";
		...
		// if ($row11['qu_initPayAmount'] != $qu_initPayAmount) {
		//	if ($upd_flag == 1) {
		//		$upd_memo .= "、";
		//	}
		//	$upd_memo .= "初回支払額";
		//	$upd_flag = 1;
		// }
		if ($row11['qu_installments'] != $_POST['qu_installments']) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "分割回数";
		...
		if ($row11['qu_deposit'] != $qu_deposit) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "頭金";
		...
		$w_qu_startDate = date('Ymd', strtotime(date($row11['qu_startDate'])));
		if ($w_qu_startDate != $qu_startDate) {
			if ($upd_flag == 1) {
				$upd_memo .= "、";
			}
			$upd_memo .= "返済開始予定年月";
		...
		// echo "upd_flag=". $upd_flag. " txt_cont=". $txt_cont. " POST:txt_cont=". $_POST['txt_cont'];

		if ($txt_cont > 3) {
			// echo "txt_cont=". $txt_cont. " txt_line=". $txt_line;
			echo "<script>alert('4回以上修正することはできません。更に修正したい場合には、新規見積もりを作成して下さい。');location.href='/manage/invoices/';</script>";
			exit();
		}
		// $in_postal = $_POST['in_postal1']. "-". $_POST['in_postal2'];
		...
		$sql = sprintf('UPDATE `quotation` SET c_id="%d", u_id="%d",
			...
			syohkei="%s", txt_cont="%d", txt_line="%d", updated=NOW(), notice_flag=0 WHERE qu_id="%d"',
			...
			mysqli_real_escape_string($db, $txt_cont),
			mysqli_real_escape_string($db, $txt_line),
			mysqli_real_escape_string($db, $_GET['id'])
		);
		mysqli_query($db,$sql) or die(mysqli_error($db));

		$q_memo = $upd_memo. $_POST['memo'];
		$sql = sprintf('UPDATE `q_memo` SET q_memo="%s" WHERE qu_id="%d"',
			mysqli_real_escape_string($db, $q_memo),
			mysqli_real_escape_string($db, $_GET['id'])
		);
		$record = mysqli_query($db, $sql) or die(mysqli_error($db));
...
// $qu_commit 	     = number_format($row7['qu_commit'], 2);
					...
					<div class="all_wrapper">
						...
						<div class="main_title_inner">
							<div class="main_title_top">
								...
								<?php } ?>
								<?php if ($errMsg == "OK") { ?>
									<font color="#f05b72">※請求データを登録しました。</font>
								<?php } ?>
							</div>
						</div>
					</div>
						...
						<div class="main_content_inner estimates_new_inner">
							<form action="" method="post" accept-charset="utf-8">
											...
											<div class="estimates_company_info">
												...
												<div class="estimates_company_info_inner">
													<table class="estimates_new_company">
															...
															<td id="client_number">
																<span class="select_box">
																	<select name="qu_custom_no" class="ajax_number">
																		<?php for($ix=0; $ix<$custom_count; $ix++) { ?>
																			<?php if ($row7['qu_custom_no'] == $c_id[$ix]) { ?>
														...
														<!-- <tr>
															<th class="must_form">対象分類</th>
															<td id="target_type"><input class="calculation_area" type="text" name="qu_bunrui" value="<?php echo $qu_bunrui; ?>"></td>
														</tr> -->
														<tr>
															<th id="item_name" class="must_form"><span><?php echo $qu_bunrui; ?></span>名称</th>
										...
										<div class="estimates_new_content_inner">
											<div class="estimates_table_inner">
												...
												<div class="estimates_table">
													<div class="estimates_table_inner">
														<p class="company_title sp">請求詳細</p>
														<div class="estimates_table_field">
															<div class="estimates_table_box">

																<table class="estimates_table_title pc">
																	<tr>
																		<th><span class="total_title">請求金額合計</span><span class="total_nums"><input class="calculation_area total_area" type="text" name="q_alltotal" v-model="estimatesPrice" readonly="readonly" title="自動計算エリア"></span></th>
																...
																<table class="estimates_table_detail">
																	...
																	<tr class="estimates_table_time">
																		...
																		<!-- <td class="time_left time_input">
																			<span class="select_box"><input class="text-center" type="text" name="qu_startDate" v-model="paymentStart">
																			</span>
																		</td> -->
																		<td class="time_left time_input">
																				<!-- <input class="text-center" type="text" name="qu_startDate" v-model="paymentStart"> -->
																			<vuejs-datepicker
																		...
																		<th class="time_right time_name">返済終了予定年月</th>
																		<td class="time_right time_input calculation_area"><input class="text-right calculation_area sp-text-left" type="text" name="qu_endDate" value="" readonly="readonly" v-model="paymentFinish" title="自動計算エリア"></td>
																	</tr>
																</table>
															</div>
														</div>
													...
													<div class="estimates_table_inner">
														<p class="company_title sp">請求項目</p>
														<div class="estimates_table_field">
																		...
																		<tr v-for="(list, i) in lists" :key="i" v-cloak>
																			...
																			<td>
																				...
																				<span class="item_detail_name">
																					<input type="text" v-bind:name="['item-name' + i]" v-model="list.name" placeholder="請求項目を記入" maxlength="50"  title="50文字以内">
																				</span>
																				<span class="item_detail_name">
																					<div class="flex-box">
																					  <div class="flex-item">
																					    <span class="select_box2">
																					      <select style="border-radius:5px; background:#f5f5f5;" :name="['asset' + i]" v-model="list.asset" v-on:change="fetchAsset(i)">
																					  ...
																					  <div class="flex-item">
																					    <span class="select_box2">
																					      <select style="border-radius:5px; background:#f5f5f5;" :name="['use' + i]" v-model="list.use" v-on:change="fetchUse(i)" :class="status(lists[i].use_required)">
																					  ...
																					  <div class="flex-item">
																					    <span class="select_box2">
																					      <select style="border-radius:5px; background:#f5f5f5;" :name="['detail' + i]" v-model="list.label" v-on:change="fetchLabel(i)" :class="status(lists[i].label_required)">
																					...
																					<div>
																					  <input type="hidden" style="border-radius:5px; background: #f5f5f5;" :name="['life' + i]" :value="list.life">
																					</div>
																				</span>
																			</td>
																			<td>
																			...
																			<td>
																				...
																				<span class="item_detail_name">
																					<!-- <input class="text-right dot_area" type="text" v-bind:name="['item-total' + i]" v-model="lists[i].sum" placeholder="金額を記入"> -->
								...
								<div class="common_submit_button2">
									<input type="submit" name="send2" class="btn btn-info" value="請求書反映">
								</div>
			...
			var estimateCalculate = new Vue ({
				...
				data:function () {
					return {
						...
						adminPer:       "<?= h($qu_commission); ?>",	   		// 事務管理手数料率
						// initialPayment: "¥<?= h($qu_initPayAmount); ?>", 		// 初回お支払額
						downPayment:    "¥<?= h($qu_deposit); ?>",			// 頭金
						numberPayments: "<?= h($row7['qu_installments']); ?>",  	// 分割回数
						paymentStart:   "<?= h($qu_startDate); ?>",			// 分割予定開始年月
						deliveryDate:   "<?= h($row7['qu_deliveryDate']); ?>",		// 受渡期日
						lists: [
			...
			//	},
				...
				computed: {
					...
					monthlyPayment: function() {
						let estiPay = Number(this.estimatesPrice.replace(/[^0-9]/g, ''));	// 請求合計
