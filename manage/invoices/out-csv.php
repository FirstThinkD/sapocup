...
if ($_SESSION['cl_chkbox'][0] == -1) {
	// echo "cl_chkbox ALL". "<br>";
	$sql = sprintf('SELECT * FROM `quotation` WHERE u_id="%d" AND sms_flag=1
		AND delFlag=0',
		mysqli_real_escape_string($db, $_SESSION['loginID'])
	);
} else if ($_SESSION['cl_chkbox'][0] > 0) {
	// echo "cl_chkbox". $_SESSION['cl_chkbox'][0]. "<br>";
	$sql = 'SELECT * FROM `quotation` WHERE ';
	for($ix=0; $ix<$_SESSION['cl_chkbox'][0]; $ix++) {
		// echo "cl_chkbox=". $_SESSION['cl_chkbox'][($ix + 1)]. "<br>";
		$sql .= 'qu_id="'. $_SESSION['cl_chkbox'][($ix + 1)]. '"';
		if ($_SESSION['cl_chkbox'][0] > ($ix + 1)) {
			$sql .= ' OR ';
		}
	}
}

if ($_SESSION['cl_chkbox'][0] != 0) {
...
$dfile_path = "/usr/home/haw1008ufet9/html/manage/invoices/csv/". $dfile_name;
...
$ddata = "請求書番号,名称,請求先,支払日,受渡期日,総金額\n";
if ($browser == "sp") {
	$str = pack('C*',0xEF,0xBB,0xBF). $ddata;
} else {
	$str = mb_convert_encoding($ddata, "SJIS", "UTF-8");
