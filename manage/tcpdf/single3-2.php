...
$pageCnt = 0;
$pageCnt = $pdf->setSourceFile(__DIR__ . '/pdf/single_top.pdf');
for( $i=1 ; $i<=$pageCnt ; $i++ ){
	$pdf->addPage();
...
if ($_SESSION['da_pdfbox'][0][3] == 0) {
	$pdf->Cell(0, 4, "見積書（控え）" , 0, 0, 'C');
} else {
	$pdf->Cell(0, 4, "請求書（控え）" , 0, 0, 'C');
}
// 日付
...
$pdf->Cell(0, 4, $contract_c_attr , 0, 0, 'L');

$pdf->SetXY(17.0, 43.0);
$pdf->SetFont('kozminproregular', '', 8);
if ($_SESSION['da_pdfbox'][0][3] == 0) {
	$pdf->Cell(60, 4, "下記の通り御見積り申し上げ、手配いたします。" , 0, 0, 'L');
} else {
	$pdf->Cell(60, 4, "下記の通り御請求申し上げ、手配いたします。" , 0, 0, 'L');
}
// 請求情報
$pdf->SetXY(17.0, 50.0);
if ($contract_bunrui == "工事") {
	$pdf->Cell(22, 4, "工事名称" , 0, 0, 'L');
} else {
	$pdf->Cell(22, 4, "商品名称" , 0, 0, 'L');
}
...
if ($contract_bunrui == "工事") {
	...
	$pdf->Cell(22, 4, "納品場所" , 0, 0, 'L');
}
$pdf->Cell(0, 4, $contract_place , 0, 0, 'L');

$pdf->MultiCell( 0, 4, "支払条件", 0, 'J', 0, 1, 17, 70, true, false, false, true, 5, 'T');
...
$pdf->MultiCell( 0, 4, "分割払い", 0, 'J', 0, 1, 39, 70, true, false, false, true, 5, 'T');
...
$pdf->SetFont('kozminproregular', '', 8);
$name_num = count($item_name); // 配列の中身を計算
...
$pdf->SetCellPaddings(0.3, 2); // パディング設定
$num_num = count($item_num);
for ($i = 0; $i < $num_num; $i++) {
...
$pdf->SetCellPaddings(.3, 2.5, .3, 2.5); // パディング設定
// $pdf->SetFont('kozminproregular', '', 5);
$pdf->SetFont('kozminproregular', '', 7);
$unit_num = count($item_unit);
for ($i = 0; $i < $unit_num; $i++) {
	if ($item_unit[$i] == "立方メートル") {
		$pdf->SetFont('kozminproregular', '', 5);
	} else {
		$pdf->SetFont('kozminproregular', '', 7);
	}
...
$pdf->SetFont('kozminproregular', '', 7);
$pdf->SetCellPadding(0.1); // パディング設定
$pdf->MultiCell( 17.6, 0, "単価(税抜き)", 0, 'C', 1, 1, 131, 146, true, false, false, true, 5, 'M');
$pdf->SetCellPaddings(.5, 2, .5, 2); // パディング設定
...
--------------------- */
$pdf->SetCellPadding(0.1); // パディング設定
$pdf->MultiCell( 26.4, 0, "金額(税抜き)", 0, 'C', 1, 1, 148.6, 146, true, false, false, true, 5, 'M');
$pdf->SetCellPaddings(.5, 2, .5, 2); // パディング設定
...
/*	小計
--------------------- */
$pdf->SetXY(105.0, 226.0);
$pdf->SetFont('kozminproregular', 'B', 8);
$pdf->Cell(44, 7.7, "小計" , 0, 0, 'C');
$pdf->Cell(26.4, 7.7, $table_subtotal , 0, 0, 'C');
$pdf->SetXY(105.0, 229.0);
$pdf->SetFont('kozminproregular', '', 7);
$pdf->Cell(44, 7.7, "(内消費税)" , 0, 0, 'C');
$pdf->Cell(26.4, 7.7, "(". $table_tax. ")" , 0, 0, 'C');

/*	備考
--------------------- */
$pdf->SetFont('kozminproregular', '', 7);
$pdf->SetCellPaddings(1, 1, 1, 0);
$pdf->MultiCell( 141, 5, "【備考】", 0, 'L', 1, 1, 34, 234.7, true, false, false, true, 5, 'T');
$pdf->SetCellPaddings(1, 0, 1, 1);
// $pdf->MultiCell( 141, 14, $item_remark, 0, 'L', 1, 1, 34, "", true, false, false, true, 5, 'T');
$pdf->MultiCell( 141, 14, $item_remark, 0, 'L', 1, 1, 34);

/*	注意事項
--------------------- */
$pdf->MultiCell( 70, 10, "上記案内及び裏面「割賦販売契約約款」を確認し、説明を受けて、支払い方法や金額及び自らの支払い能力等を十分に検討したうえで商品の発注をします。", 0, 'L', 0, 2, 17, 270, true);

/*	注意事項
--------------------- */
$pdf->SetXY(117.7, 270.0);
