...
if (!empty($_POST['send2']) && $_POST['send2'] == "パターンを更新する") {
	...
	$qu_price 	= preg_replace('/[^0-9]/','', $_POST['qu_price']);			// 工事商品価格の「¥」を取る
	...
	header("Location:/manage/estimates/simulation/?id=". $qu_id);
	exit();
}
?>
				...
				<div class="pankuzu">
					<div class="container">
						<ul>
							...
							<li><a href="/manage/estimates/simulation/?id=<?php echo $qu_id; ?>"><span>シミュレーション</span></a></li>
										...
										<table>
											<thead>
												<!-- <tr>
													<th></th>
													<th>日付</th>
													<th><input type="text" name="qu_startDate" value="<?php echo $qu_startDate; ?>"></th>
													<th></th>
													<th></th>
												</tr> -->
												<tr class="color">
													<th class="text-center">
														...
														<span class="number"><input class="text-center" type="text" name="qu_price" v-model="shokei" @focus="focusShokei" @blur="blurShokei"></span>
													</th>
													<th>パターン1</th>
													<th>パターン2</th>
													<th>パターン3</th>
    	...
    	(function() {
    		...
    		var simulationEdit = new Vue ({
    			...
    			data:function () {
					return {
						shokei: "¥<?= h($qu_price); ?>",
						downPayment1: "¥<?= h($pt1_deposit); ?>",    			// 頭金
						downPayment2: "¥<?= h($pt2_deposit); ?>",    			// 頭金
						downPayment3: "¥<?= h($pt3_deposit); ?>",   			// 頭金
						adminPer1: "<?= h($row7['pt1_commission']); ?>",  		// 事務管理手数料率
						adminPer2: "<?= h($row7['pt2_commission']); ?>",  		// 事務管理手数料率
						adminPer3: "<?= h($row7['pt3_commission']); ?>",  		// 事務管理手数料率
						numberPayments1: "<?= h($row7['pt1_installments']); ?>", // 分割回数
						numberPayments2: "<?= h($row7['pt2_installments']); ?>", // 分割回数
						numberPayments3: "<?= h($row7['pt3_installments']); ?>", // 分割回数
					}
    			},
    			// 要素を追加する機能
    			methods: {
