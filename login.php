...
if (!empty($_POST['login2']) && $_POST['login2'] == "ログイン") {
	...
	$sql = sprintf('SELECT COUNT(*) AS user_cont FROM user WHERE (u_id="%s") AND (u_pass="%s" OR p_pass="%s") AND delFlag=0',
	...
	if ($row0['user_cont'] == 1) {
		$sql = sprintf('SELECT * FROM user WHERE (u_id="%s") AND (u_pass="%s" OR p_pass="%s") AND delFlag=0',
			mysqli_real_escape_string($db, $_POST['loginID']),
			mysqli_real_escape_string($db, $passwd),
			mysqli_real_escape_string($db, $passwd)
		);
		...
		// ログイン失敗
		$sql = sprintf('SELECT * FROM `user` WHERE (u_id="%s") AND delFlag=0',
			mysqli_real_escape_string($db, $_POST['loginID'])
		);
		$record = mysqli_query($db, $sql) or die(mysqli_error($db));
					...
					<div class="otherContentInner loginWrap">
						<div class="loginBox">
							<?php if ($signError == "failEmail") { ?>
								<font color="#f05b72">ログインIDまたはパスワードが一致しません。</font>
								<br><br>
							<?php } ?>
							<?php if ($signError == "failPass") { ?>
								<font color="#f05b72" style="white-space: nowrap">5回連続でログインに失敗したため、パスワードの再設定をお願いします。</font>
								<br><br>
							<?php } ?>
							<div class="inputBox">
								<input type="text" class="form-control" name="loginID" placeholder="ログインID入力" required>
							</div>
							<div class="inputBox">
								<input type="password" id="password" class="form-control" name="passwd" placeholder="パスワード入力" required>
								<input type="checkbox" id="passcheck" /><label for="passcheck">パスワードを表示</label>
							</div>
							<div class="inputBox inputPass">
								<input type="checkbox" id="pass_save" /><label for="pass_save">パスワードを保存する</label>
							</div>
