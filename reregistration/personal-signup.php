...
if (!empty($_POST['next2']) && $_POST['next2'] == "確認画面へ") {
	...
	if ($signError == "") {
		...
		$sql = sprintf('SELECT * FROM user WHERE delFlag=0 AND
			(u_email="%s" OR p_email="%s") AND (u_pass="%s" OR p_pass="%s")',
					...
					<div class="loginBox">
						<form action="" method="post">
							...
							<?php if ($signError == "failEmail2") { ?>
								<font color="#f05b72">重複するメールアドレスの場合、異なるパスワードを設定してください。</font>
							<?php } ?>
							...
							<div class="inputBox">
								...
								<input type="text" class="form-control" name="p_name" value="<?php if(!empty($_SESSION['p_name'])) {echo h($_SESSION['p_name']);} ?>" placeholder="さぽかっぷ" required>
							</div>
							<div class="inputBox">
								<p>名前（カタカナ）</p>
									...
									<div class="birthBox year cf">
										<span class="birth_input select_box">
											<select v-cloak v-model="year" @change="get_days" class="form-control" name="p_year" required>
											  <option v-for="n in 80" :value="n + 1920">{{ n + 1920 }}</option>
							...
							<div class="inputBox">
								<p>携帯電話<span class="must">必須</span></p>
								<input type="text" class="form-control" name="p_tel" value="<?php if(!empty($_SESSION['p_tel'])) {echo h($_SESSION['p_tel']);} ?>" placeholder="09012345678" required>
							</div>
							<div class="inputBox">
								<p>郵便番号<span class="must">必須</span></p>
								<div class="inputBoxAddress cf">
									<div class="addressBox post">
										<input type="text" name="p_postal1" value="<?php echo $_SESSION['p_postal1']; ?>" maxlength="3" placeholder="123" required>-<input type="text" name="p_postal2" value="<?php echo $_SESSION['p_postal2']; ?>" maxlength="4" onKeyUp="AjaxZip3.zip2addr('p_postal1','p_postal2','p_address1','p_address2','p_address2');" placeholder="4567" required>
							...
							<div id="habbitButton" class="inputBox submitBox" v-bind:class="{ no_button: isActive }">
								<input type="submit" name="next2" value="確認画面へ" style="cursor:pointer;">
							</div>
						</form>
					</div>
				</div>
