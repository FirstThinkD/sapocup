...
if (!empty($_POST['next2']) && $_POST['next2'] == "確認画面へ") {
	...
	$_SESSION['p_kana']     = $_POST['p_kana'];
	$_SESSION['p_year']     = $_POST['p_year'];
	$_SESSION['p_mouth']    = $_POST['p_mouth'];
	$_SESSION['p_day']      = $_POST['p_day'];
	$_SESSION['p_tel']      = $_POST['p_tel'];
	$_SESSION['p_tel2']      = $_POST['p_tel2'];
	$_SESSION['p_postal1']  = $_POST['p_postal1'];
	$_SESSION['p_postal2']  = $_POST['p_postal2'];
	$_SESSION['p_address1'] = $_POST['p_address1'];
	$_SESSION['p_address2'] = $_POST['p_address2'];
	$_SESSION['p_address3'] = $_POST['p_address3'];
	$_SESSION['p_email1']   = $_POST['p_email1'];
	$_SESSION['p_email2']   = $_POST['p_email2'];
	$_SESSION['p_pass1']    = $_POST['p_pass1'];
	$_SESSION['p_pass2']    = $_POST['p_pass2'];

	if ($_SESSION['p_email1'] != $_SESSION['p_email2']) {
		$signError = "failEmail";
	}

	if ($signError == "") {
		if ($_SESSION['p_pass1'] != $_SESSION['p_pass2']) {
			$signError = "failPass";
		}
	}
	...
	if ($signError == "") {
        ...
        //お電話番号※Check
        if (strlen($_SESSION['p_tel'])== 0 && strlen($_SESSION['p_tel2']) ==0) {
            $error = "tel_exist";
        }
        //携帯番号Check
        if (strlen($_SESSION['p_tel'])>0 && strlen($_SESSION['p_tel']) != 11){
            $error = "tel_no";
        }
        if (strlen($_SESSION['p_tel'])== 11 && !(bool) preg_match('/^[\-+]?[0-9]+$/', $_SESSION['p_tel']) && false === strpos($_SESSION['p_tel'], "090") && false === strpos($_SESSION['p_tel'], "080") && false === strpos($_SESSION['p_tel'], "070")) {
            $error = "tel_format";
        }
        //固定電話番号Check
        if (strlen($_SESSION['p_tel2'])>0 && strlen($_SESSION['p_tel2']) != 10){
            $error = "tel_no2";
        }
        if (strlen($_SESSION['p_tel2'])== 10 && !(bool) preg_match('/^[\-+]?[0-9]+$/', $_SESSION['p_tel2']) ) {
            $error = "tel_format2";
        }
	}
		...
		<div class="otherScreen help">
			<div class="allWrapper">
				<div class="otherScreenInner">
					<h1>会員登録（個人）</h1>
                    <span>※オプションにつきましては登録後にサービスの追加が可能です。</span>
				</div>
			</div>
		</div>
		<div id="memberRegistation" class="otherContent">
			<div class="allWrapper">
					...
					<div class="loginBox">
						<form action="" method="post">
							...
							<?php if ($signError == "tel_no") { ?>
								<font color="#f05b72">携帯電話番号欄は、正しい形式ではありません。</font>
							<?php } ?>
							<?php if ($signError == "tel_no2") { ?>
								<font color="#f05b72">固定電話番号欄は、正しい形式ではありません。</font>
							<?php } ?>
							<?php if ($signError == "tel_exist") { ?>
								<font color="#f05b72">お電話番号欄は必須です。</font>
							<?php } ?>
                            <?php if ($signError == "tel_format") { ?>
                                <p class="error_message">携帯電話番号のみ11桁を指定してください。</p>
                            <?php } ?>
                            <?php if ($signError == "tel_format2") { ?>
                                <p class="error_message">固定電話番号のみ10桁を指定してください。</p>
                            <?php } ?>

							<div class="inputBox">
								<p>名前<span class="must">必須</span></p>
								<input type="text" class="form-control" name="p_name" value="<?php if(!empty($_SESSION['p_name'])) {echo h($_SESSION['p_name']);} ?>" required placeholder="さぽかっぷ">
							</div>
							<div class="inputBox">
								<p>名前（カタカナ）</p>
								<input type="text" class="form-control" name="p_kana" value="<?php if(!empty($_SESSION['p_kana'])) {echo h($_SESSION['p_kana']);} ?>" pattern="[\u30A1-\u30F6,　,ー,]*" placeholder="サポカップ">
							</div>
							<div class="inputBox">
								<p>生年月日<span class="must">必須</span></p>
								<div id="selectDate" class="inputBoxBirth cf">
									<div class="birthBox year cf">
										<span class="birth_input select_box">
											<select v-cloak v-model="year" @change="get_days" class="form-control" name="p_year" required>
											  <option v-for="n in <?php echo $years; ?>" :value="n + <?php echo $birth_top; ?>">{{ n + <?php echo $birth_top; ?> }}</option>
											   <?php if ($_SESSION['p_year'] != "") { ?>
												<option value="<?php if(!empty($_SESSION['p_year'])) {echo h($_SESSION['p_year']);}?>" selected><?php if(!empty($_SESSION['p_year'])) {echo h($_SESSION['p_year']);} ?></option>
											   <?php } ?>
											</select>
										</span>
									...
									<div class="birthBox month cf">
										<span class="birth_input select_box">
											<select v-cloak v-model="month" @change="get_days" class="form-control" name="p_mouth" required>
												<option v-for="n in 12" :value="n" v-cloak>{{ n }}</option>
												<?php if ($_SESSION['p_mouth'] != "") { ?>
												<option value="<?php if(!empty($_SESSION['p_mouth'])) {echo h($_SESSION['p_mouth']);} ?>" selected><?php if(!empty($_SESSION['p_mouth'])) {echo h($_SESSION['p_mouth']);} ?></option>
											<?php } ?>
											</select>
										</span>
									...
									<div class="birthBox day cf">
										<span class="birth_input select_box">
											<select v-cloak v-model="day" class="form-control" name="p_day" required>
												<option v-for="n in days_max" :value="n">{{ n }}</option>
												<?php if ($_SESSION['p_day'] != "") { ?>
												<option value="<?php if(!empty($_SESSION['p_day'])) {echo h($_SESSION['p_day']);} ?>" selected><?php if(!empty($_SESSION['p_day'])) {echo h($_SESSION['p_day']);} ?></option>
											<?php } ?>
											</select>
										</span>
							...
							<div class="inputBox">
								<p>携帯電話</p>
								<input type="text" class="form-control" name="p_tel" value="<?php if(!empty($_SESSION['p_tel'])) {echo h($_SESSION['p_tel']);} ?>" maxlength="11" placeholder="09012345678" title="半角数字11文字をご入力ください" >
							</div>
							<div class="inputBox">
								<p>固定電話番号</p>
								<input type="text" class="form-control" name="p_tel2" value="<?php if(!empty($_SESSION['p_tel2'])) {echo h($_SESSION['p_tel2']);} ?>" maxlength="10" placeholder="0330001234" title="半角数字11文字をご入力ください" >
							</div>
							<div class="inputBox">
								<p>郵便番号<span class="must">必須</span></p>
								<div class="inputBoxAddress cf">
									<div class="addressBox post">
										<input type="text" class="form-control postalInput" name="p_postal1" value="<?php echo $_SESSION['p_postal1']; ?>" maxlength="3" placeholder="123" required>-<input type="text" class="postalInput" name="p_postal2" value="<?php echo $_SESSION['p_postal2']; ?>" maxlength="4" onKeyUp="AjaxZip3.zip2addr('p_postal1','p_postal2','p_address1','p_address2','p_address2');" placeholder="4567" required>
										<!-- <input type="text" class="form-control" name="p_postal" value="<?php if(!empty($_SESSION['p_postal'])) {echo h($_SESSION['p_postal']);} ?>" placeholder="1234567" size="10" maxlength="8" onKeyUp="AjaxZip3.zip2addr(this,'','p_address1','p_address2');" required> -->
									</div>
									<div class="addressBox">
										<p>都道府県<span class="must">必須</span></p>
										<input type="text" class="form-control" name="p_address1" value="<?php if(!empty($_SESSION['p_address1'])) {echo h($_SESSION['p_address1']);} ?>" placeholder="東京都"  required>
									</div>
									<div class="addressBox">
										<p>市区町村<span class="must">必須</span></p>
										<input type="text" class="form-control" name="p_address2" value="<?php if(!empty($_SESSION['p_address2'])) {echo h($_SESSION['p_address2']);} ?>" required>
									</div>
									<div class="addressBox">
										<p>番地<span class="must">必須</span></p>
										<input type="text" class="form-control" name="p_address3" value="<?php if(!empty($_SESSION['p_address3'])) {echo h($_SESSION['p_address3']);} ?>" required>
									</div>
								</div>
							</div>
                            <div class="inputBox">
								<p>ご登録をご希望のメールアドレス<span class="must">必須</span></p>
								<input type="email" class="form-control" name="p_email1" value="<?php if(!empty($_SESSION['p_email1'])) { echo h($_SESSION['p_email1']);} ?>" required>
							</div>
							<div class="inputBox">
								<p>ご登録をご希望のメールアドレス※確認用<span class="must">必須</span></p>
								<input type="email" class="form-control" name="p_email2" value="<?php if(!empty($_SESSION['p_email2'])) { echo h($_SESSION['p_email2']);} ?>" required>
							</div>
							<div class="inputBox">
								<p>パスワード(半角英数字８文字以上)<span class="must">必須</span></p>
								<input type="password" id="password1" class="form-control" name="p_pass1" value="<?php if(!empty($_SESSION['p_pass1'])) { echo h($_SESSION['p_pass1']);} ?>" minlength="8" required>
								<input type="checkbox" id="passcheck1" /> パスワードを表示
							</div>
							<div class="inputBox">
								<p>パスワード※確認用(半角英数字８文字以上)<span class="must">必須</span></p>
								<input type="password" id="password2" class="form-control" name="p_pass2" value="<?php if(!empty($_SESSION['p_pass2'])) { echo h($_SESSION['p_pass2']);} ?>" minlength="8" required>
							...
							<div class="modalPanel">
                                ...
                                <div>
                                    <br/>
                                    <input type="checkbox" id="is_agree" name="is_agree" value="Two" v-model="isActive">
                                    <label for="is_agree">利用規約を読んで同意する<span class="must">必須</span></label>
                                </div>
                            </div>
