...
if ($_POST['token'] != "") {
	...
	$data .= " customer_id=". $pay_customer_id. " running_id=xxxxxxx";
	$data .= " kind=025 resultStatus=". $resultStatus;
	$data .= "\n";
	...
	if ($resultStatus == 1 || $resultStatus == "") {
		...
		echo "<script>alert('カード情報登録でエラーが発生しました。左上のエラー内容を管理者にお伝えください。');location.href='/paygent/';</script>";
		exit();
	} else {
