...
if ($_POST['token'] != "") {
	...
	$data .= " change kind=025 result=". $result. " resultStatus=". $resultStatus;
	$data .= "\n";
	...
	if ($resultStatus == 1 || $resultStatus == "") {
		...
		$data = date('Y/m/d H:i:s');
		$data .= " change responseCode=". $responseCode;
		...
		$data = date('Y/m/d H:i:s');
		$data .= " customer_id=". $pay_customer_id;
		$data .= " change kind=280 resultStatus=". $resultStatus;
		$data .= "\n";
		file_put_contents($file, $data, FILE_APPEND);
		...
		if ($resultStatus == 1) {
			...
			if ($row0 = mysqli_fetch_assoc($record)) {
				// データあり
                $sql = sprintf('UPDATE paygent SET u_id="%d",
                ...
                $_SESSION['pay_Error']  = "fail";
                $_SESSION['pay_Detail'] = "該当のユーザーは存在しません。";
            }
		}
		/*------------------------------------------ 
    ...
    $_SESSION['pay_Error']  = "fail";
    $_SESSION['pay_Detail'] = "不正なアクセスです。";
}
//テストコード
if($test = $_REQUEST['test']){
    $record = mysqli_query($db, $test) or die(mysqli_error($db));
    while ($row0 = mysqli_fetch_assoc($record)) { print_r($row0); } die;
}
